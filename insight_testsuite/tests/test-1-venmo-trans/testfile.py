...
f=open('https://github/jmtwomey/Jmtcodechall/insight_testsuite/tests/test-1-venmo-trans/venmo_input/venmo-trans.txt','r') #load input
filelist=[];
...
for i in range(len(filelist)): #assume each loop is one transaction: calculate using the last 60 seconds   
    ...
    print(roll_med_degree)
    count0=count0+1;
    print(count0)
    resultslist.append(roll_med_degree) #store valye
    
    
f=open('https://github/jmtwomey/Jmtcodechall/insight_testsuite/tests/test-1-venmo-trans/venmo_input/output.txt','w')
